...
const Navbar = () => {
  const { logAuth, isAuthenticated, employerAuth } = useSelector(
    (state) => state.auth,
  );
    ...
    <nav className="fixed top-0 left-0 right-0 w-full z-50 bg-white border-b border-slate-100 h-16 flex items-center">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between w-full">
        <Link
          ...
          className="text-xl sm:text-2xl font-bold text-[#0a66c2] tracking-tighter"
        >
        ...
        <div className="hidden md:flex items-center gap-6">
          <Link
            to="/"
            className="text-sm font-medium text-slate-600 hover:text-[#0a66c2]"
          >
            Home
          </Link>
          <Link
          ...
          {(isAuthenticated || employerAuth) && (
            <NavLink
              to={employerAuth ? "/employer-dashboard" : "/candidate-dashboard"}
              className="text-sm font-medium text-slate-600 hover:text-[#0a66c2]"
            >
              Dashboard
            </NavLink>
        ...
        <div className="hidden md:flex items-center gap-3">
          {!logAuth ? (
            <>
              ...
              <Link
                to="/register-email"
                className="bg-[#0a66c2] text-white text-sm font-semibold px-6 py-2 rounded-lg hover:bg-[#084d91] transition"
              >
                Register
              </Link>
            </>
        ...
        <div className="md:hidden absolute top-16 left-0 right-0 bg-white border-b border-slate-100 shadow-lg px-6 py-4 space-y-4">
          ...
          {!logAuth ? (
            <div className="pt-4 border-t flex flex-col gap-3">
              <Link
                to="/login"
                className="text-center py-2 text-slate-700 font-semibold"
                onClick={() => setIsMenuOpen(false)}
              >
                Log in
              </Link>
              <Link
                to="/register-email"
                className="text-center bg-[#0a66c2] text-white py-2 rounded-lg font-semibold"
                onClick={() => setIsMenuOpen(false)}
              >
                Register
              </Link>
